...
gan_loss_type=SSIM
folder_name="new_saves/comparison"
bottleneck_option=None
for random_seed in $random_seed_list; do
        for cutlayer in $cutlayer_list; do
                for num_agent in $num_agent_list; do
                        filename=ace_${scheme}_${arch}_cutlayer_${cutlayer}_client_${num_agent}_seed${random_seed}_dataset_${dataset}_lr_${learning_rate}_${regularization}_both_${train_gan_AE_type}_${regularization_strength}_${num_epochs}epoch_bottleneck_${bottleneck_option}
                        ...
                        regularization=gan_adv_noise_step1
                        regularization_strength_list="0.05 0.08 0.10"
                        ...
                        for regularization_strength in $regularization_strength_list; do
                                ...
                                CUDA_VISIBLE_DEVICES=${GPU_id} python main_test_MIA.py --arch=${arch} --cutlayer=$cutlayer --batch_size=${batch_size} \
                                        --filename=$filename --num_agent=$num_agent --num_epochs=$num_epochs \
                                        --dataset=$dataset --scheme=$scheme --target_agent=${target_agent} --new_log_folder\
