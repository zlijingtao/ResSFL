...
folder_name="supple_saves/finetune_noniid"
...
noniid_ratio_list="0.1 0.2 0.5"
source_task_list="cifar10"
dataset=cifar100
learning_rate=0.05
local_lr_list="0.005"
                ...
                for cutlayer in $cutlayer_list; do
                        for num_client in $num_client_list; do
                                for local_lr in $local_lr_list; do
                                        for transfer_source_task in $source_task_list; do
                                                for noniid_ratio in $noniid_ratio_list; do
                                                filename=ace_${scheme}_${arch}_cutlayer_${cutlayer}_client_${num_client}_seed${random_seed}_dataset_${dataset}_noniid_${noniid_ratio}_lr_${learning_rate}_${regularization}_both_${train_gan_AE_type}_${regularization_strength}_${num_epochs}epoch_bottleneck_${bottleneck_option}_servertune_${local_lr}_loadserver_source_${transfer_source_task}
                                                CUDA_VISIBLE_DEVICES=${GPU_id} python main_MIA.py --arch=${arch}  --cutlayer=$cutlayer --batch_size=${batch_size} \
                                                        ...
                                                        --dataset=$dataset --noniid ${noniid_ratio} --scheme=$scheme --regularization=${regularization} --regularization_strength=${regularization_strength}\
