...
bc_list="8"
gan_loss_type=SSIM
folder_name="new_saves/smartMIA"
                        ...
                        for regularization_strength in $regularization_strength_list; do
                                for cutlayer in $cutlayer_list; do
                                        for num_agent in $num_agent_list; do
                                                filename=ace_${scheme}_${arch}_cutlayer_${cutlayer}_client_${num_agent}_seed${random_seed}_dataset_${dataset}_lr_${learning_rate}_${regularization}_both_${train_gan_AE_type}_${regularization_strength}_${num_epochs}epoch_bottleneck_${bottleneck_option}_ssim_${ssim_threshold}_advtrain
                                                CUDA_VISIBLE_DEVICES=${GPU_id} python main_MIA.py --arch=${arch}  --cutlayer=$cutlayer --batch_size=${batch_size} \
                                                        --filename=$filename --num_agent=$num_agent --num_epochs=$num_epochs --save_more_checkpoints\
                                                ...
                                                for attack_from_later_layer in ${attack_from_later_layer_list}; do
                                                        for num_epochs in $num_epochs_list; do
                                                                ...
                                                                CUDA_VISIBLE_DEVICES=${GPU_id} python main_test_MIA.py --arch=${arch} --cutlayer=$cutlayer --batch_size=${batch_size} \
                                                                        ...
                                                                        --dataset=$dataset --scheme=$scheme --target_agent=${target_agent} --attack_from_later_layer $attack_from_later_layer\
